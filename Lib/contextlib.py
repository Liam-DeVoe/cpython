...
class _PerThreadStream:
    def __init__(self, default_stream):
        ...
        # each stack entry is (stream, thread_id) where thread_id is None
        # when per_thread=False
        self._stack = []
...
class _RedirectStream(AbstractContextManager):
    ...
    def __enter__(self):
        with self._lock:
            ...
            self._stream_ref.add_entry(self._entry)

        return self._new_target

    def __exit__(self, exctype, excinst, exctb):
        with self._lock:
            self._stream_ref.remove_entry(self._entry)
